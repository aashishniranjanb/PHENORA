...
const STAGES: { id: StageId; label: string; sub: string }[] = [
  ...
  { id: "TWIN", label: "TWIN", sub: "State Representation" },
...
export default function ExperimentTimeline({
  ...
  return (
    <div className="bg-white border-b border-slate-200 px-4 py-2 shrink-0 overflow-x-auto scrollbar-thin scrollbar-thumb-slate-700">
      <div className="flex items-center space-x-2 min-w-max">
        {STAGES.map((st, idx) => {
          const state = getStageState(st.id);
          const isSelected = activeStage === st.id;

          let badgeColor = "bg-slate-100 text-slate-500 border-slate-300";
          let icon = "🔒";

          if (state === "COMPLETE") {
            badgeColor = "bg-emerald-100/70 text-emerald-700 border-emerald-600/60";
            icon = "✓";
          } else if (state === "ACTIVE" || state === "PROCESSING") {
            badgeColor = "bg-emerald-100 text-emerald-700 border-emerald-400 animate-pulse";
            icon = "◉";
          } else if (state === "READY") {
            badgeColor = "bg-slate-100 text-slate-800 border-slate-600";
            icon = "○";
          }

          if (isSelected) {
            badgeColor += " ring-2 ring-emerald-400 ring-offset-2 ring-offset-[#0B1528]";
          }
          ...
          return (
            <React.Fragment key={st.id}>
              {idx > 0 && (
                <div
                  className={`h-0.5 w-6 ${
                    state === "COMPLETE" || isSelected ? "bg-emerald-500/60" : "bg-slate-100"
                  }`}
                />
              )}
              <button
                onClick={() => onSelectStage(st.id)}
                className={`flex flex-col items-start px-3 py-1.5 rounded-lg border text-left font-mono transition-all hover:border-emerald-400/80 ${badgeColor}`}
              >
                <div className="flex items-center space-x-1.5 font-bold text-[11px] tracking-wide">
                  <span>{icon}</span>
                  <span>{st.label}</span>
                </div>
                <span className="text-[9px] text-slate-600 mt-0.5">{st.sub}</span>
              </button>
            </React.Fragment>
          );
        })}
