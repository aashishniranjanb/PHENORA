  ...
  return (
    <div className="p-6 max-w-5xl mx-auto font-mono space-y-6">
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-5 flex justify-between items-center">
        <div>
          <h2 className="text-base font-bold text-slate-900 mb-1">STAGE 2 — ACQUISITION</h2>
          <p className="text-xs text-slate-600">Collecting multi-frequency bioimpedance measurements</p>
        </div>
        <div className="flex items-center space-x-3">
          <span className={`text-xs px-3 py-1 rounded font-bold ${
            isRunning ? "bg-emerald-100 text-emerald-700 border border-emerald-500 animate-pulse" : "bg-emerald-100 text-emerald-700 border border-emerald-700"
          }`}>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-5">
        <div className="flex justify-between items-center mb-4">
          <span className="text-xs text-slate-700 font-bold">TIME-DOMAIN SIGNAL STREAM</span>
          <div className="flex space-x-1 bg-white p-1 rounded border border-slate-200 text-[10px]">
            {(["RAW", "FILTERED", "RMS", "DELTA"] as const).map((mode) => (
              <button
                key={mode}
                onClick={() => setSignalView(mode)}
                className={`px-2.5 py-0.5 rounded ${
                  signalView === mode ? "bg-emerald-500 text-slate-950 font-bold" : "text-slate-600 hover:text-slate-800"
                }`}
              >
                {mode}
              </button>
        ...
        <div className="h-44 bg-slate-50 rounded-lg border border-slate-200 flex items-center justify-center relative overflow-hidden p-4">
          <svg className="w-full h-full stroke-emerald-400 fill-none" viewBox="0 0 500 100" preserveAspectRatio="none">
            <path
          ...
          <div className="absolute top-2 right-3 text-[10px] text-emerald-600 bg-white/90 border border-slate-300 px-2 py-0.5 rounded">
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-5">
        <h3 className="text-xs font-bold text-slate-700 mb-3">PROCESSING PIPELINE INDICATOR</h3>
        <div className="grid grid-cols-6 gap-2 text-[10px] text-center">
          {[
            ...
            <div key={idx} className="bg-white/90 border border-emerald-500/40 p-2.5 rounded text-emerald-700">
              <div className="font-bold mb-1">{step.label}</div>
              <div className="text-[9px] text-emerald-600">{step.status}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-4">
        <button
          ...
          className="text-xs font-bold text-slate-600 hover:text-emerald-700 flex items-center justify-between w-full"
        >
        ...
        {showEngDrawer && (
          <div className="grid grid-cols-4 gap-4 mt-4 pt-4 border-t border-slate-200 text-[11px] text-slate-700">
            <div>
              <span className="text-slate-500 block">ADC MODEL</span>
              <span className="font-bold text-emerald-700">ADS1115 (16-bit)</span>
            </div>
            <div>
              <span className="text-slate-500 block">SAMPLE RATE</span>
              <span className="font-bold text-emerald-700">860 SPS</span>
            </div>
            <div>
              <span className="text-slate-500 block">MCU INTERFACE</span>
              <span className="font-bold text-emerald-700">HELTEC ESP32-S3</span>
            </div>
            <div>
              <span className="text-slate-500 block">FPGA STATE</span>
              <span className="font-bold text-emerald-600">CALCULATE_SLOPE</span>
            </div>
          </div>
        )}
      </div>
