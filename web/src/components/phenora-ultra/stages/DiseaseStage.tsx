  ...
  return (
    <div className="p-6 max-w-5xl mx-auto font-mono space-y-6">
      ...
      <div className="bg-[#0B1528] border border-slate-800 rounded-xl p-5 flex justify-between items-center">
        <div>
          <h2 className="text-base font-bold text-white mb-1">STAGE 5 — DISEASE INTELLIGENCE</h2>
          <p className="text-xs text-slate-400">Model interpretation of bioimpedance phenotype</p>
        </div>
      ...
      {/* OOD Warning Banner if high OOD */}
      {oodScore > 30 && (
        <div className="bg-amber-950/80 border border-amber-500/60 rounded-xl p-4 text-xs text-amber-200">
          <div className="font-bold flex items-center space-x-2 text-amber-400 mb-1">
            <span>⚠ OUT-OF-DISTRIBUTION WARNING</span>
            <span className="text-[10px] bg-amber-900 px-2 py-0.5 rounded border border-amber-600">
              OOD SCORE: {oodScore}
            </span>
          </div>
          <p className="leading-relaxed text-[11px] text-amber-300/90">
            The observed phenotype is outside the validated model domain. Disease interpretation should not be treated as clinically supported. Recommended action: <strong>MEASURE AGAIN</strong>.
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-3 gap-4 text-xs">
        ...
        <div className="bg-slate-950 p-5 rounded-xl border border-slate-800 space-y-3">
          ...
          {disease?.alternatives.map((alt, i) => (
            <div key={i} className="flex justify-between items-center p-2 rounded bg-slate-900/60 text-[11px]">
      ...
      {/* Confidence Decomposition & Evidence Panel */}
      <div className="grid grid-cols-2 gap-4 text-xs">
        ...
        <div className="bg-slate-950 p-5 rounded-xl border border-slate-800 space-y-3">
          ...
          <div className="space-y-2 text-[11px] text-slate-300">
            <div className="flex justify-between"><span>Signal quality</span><span className="text-emerald-400">+{disease?.confidenceDecomposition.signalQuality}</span></div>
            <div className="flex justify-between"><span>Phenotype consistency</span><span className="text-emerald-400">+{disease?.confidenceDecomposition.phenotypeConsistency}</span></div>
            <div className="flex justify-between"><span>Model agreement</span><span className="text-emerald-400">+{disease?.confidenceDecomposition.modelAgreement}</span></div>
            <div className="flex justify-between"><span>Temporal evidence</span><span className="text-emerald-400">+{disease?.confidenceDecomposition.temporalEvidence}</span></div>
            <div className="flex justify-between"><span>Reference similarity</span><span className="text-emerald-400">+{disease?.confidenceDecomposition.referenceSimilarity}</span></div>
            <div className="flex justify-between"><span>OOD penalty</span><span className="text-red-400">{disease?.confidenceDecomposition.oodPenalty}</span></div>
          </div>
        </div>
        ...
        <div className="bg-slate-950 p-5 rounded-xl border border-slate-800 space-y-3">
          ...
          <div className="space-y-2 text-[11px]">
            {disease?.evidence.map((ev, i) => (
              <div key={i} className="p-2 bg-slate-900/70 rounded border border-slate-800 flex justify-between items-center">
                <div>
      ...
      <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 font-mono text-xs">
        ...
        >
          <span>REGISTERED MODEL SPECIFICATIONS {showModelDetails ? "▲" : "▼"}</span>
